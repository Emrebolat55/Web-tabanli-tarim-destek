...
if (isset($_GET['status']) && in_array($_GET['status'], array('open', 'closed', 'resolved'))) {
    ...
    header('Location: admin_view.php?id=' . $_GET['id']);
    exit;
}
...
<?php endif; ?>
    ...
    <div class="btns">
        <a href="admin_view.php?id=<?=$_GET['id']?>&status=closed" class="btn red">Close</a>
        <a href="admin_view.php?id=<?=$_GET['id']?>&status=resolved" class="btn">Resolve</a>
    </div>
